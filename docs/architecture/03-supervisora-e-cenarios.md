...
1. **Classificação & Roteamento:** Seleciona o especialista mais adequado a partir do catálogo resumido de 19 habilidades gerado dinamicamente por `getSkillCatalogSummary()`.
...
5. **Mensagens Intermediárias (`intermediateMessage`):** Exclusivamente em conversas interativas ao vivo com humanos, avisa proativamente o usuário antes de tarefas mais longas (ex: `"Consultando sua agenda..."`). Em **rotinas agendadas (crons / automações de sistema)**, a emissão intermediária é **estritamente suprimida** (`intermediateMessage = null`), garantindo que o pipeline execute 100% em silêncio e envie apenas o resultado final consolidado.
6. **Motor de Decisão (LLM):** Opera sobre o modelo `gpt-4o-mini` (`modelSupervisorActive`, `temperature: 0.1`), garantindo alta velocidade de resposta, fidelidade estrita ao schema JSON de roteamento e excelente fluidez conversacional.

...
- **`memoryAgent` ([`src/agents/memoryAgent.ts`](../../src/agents/memoryAgent.ts)):** Gerencia a memória cognitiva RAG no SQLite. Permite registrar novos fatos e preferências (`storeSemanticMemory` com importância), realizar buscas semânticas (`searchSemanticMemory` com reforço automático), extrair dossiês de eventos (`searchEventSummary`) e excluir memórias (`deleteSemanticMemory`).

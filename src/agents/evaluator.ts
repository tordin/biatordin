...
export const EvaluationSchema = z.object({
  verdict: z.enum(["PASS", "NEEDS_CORRECTION"]).describe(
  ...
  critique: z.object({
    isComplete: z.boolean().nullable().describe("Se todas as solicitações e perguntas do usuário foram atendidas"),
    isGrounded: z.boolean().nullable().describe("Se todas as alegações e dados apresentados têm respaldo real nas ferramentas executadas e dados coletados"),
    isPersonaCompliant: z.boolean().nullable().describe("Se o tom e estilo respeitam a persona Bia (amigável, feminina) e o formato WhatsApp"),
  }).nullable().describe("Avaliação detalhada dos critérios"),
  ...
  suggestedAction: z.enum(["ROUTE_TO_SPECIALIST", "FIX_RESPONSE_TEXT", "PASS"]).nullable().describe(
...
const EVALUATOR_SYSTEM_PROMPT = 
  ...
  "   - IMPORTANTE: Se um especialista (ex: 'whatsappAgent', 'searchAgent', 'memoryAgent', 'weatherAgent') CONSTA na lista de agentes/ferramentas executadas e há dados correspondentes em [DADOS COLETADOS NOS BASTIDORES], ENTÃO ELE FOI DE FATO EXECUTADO COM SUCESSO. NUNCA afirme que o agente não foi chamado se ele estiver listado nessas seções.\n" +
  "   - Exemplo: Se o usuário perguntou sobre menções em um grupo e o whatsappAgent executou 'getChatHistory' e o relatório diz 'Nenhuma menção encontrada', e a resposta da Bia diz 'Não encontrei nenhuma menção no grupo X', a resposta é 100% CORRETA, COMPLETA E FUNDAMENTADA -> verdict: 'PASS'.\n\n" +
  "2. QUANDO REPROVAR (verdict: 'NEEDS_CORRECTION'):\n" +
  "   - Reprove APENAS se:\n" +
  "     A) A resposta afirma ter feito algo (ex: 'agendei na agenda', 'enviei o email', 'criei a tarefa') mas o agente necessário (ex: calendarAgent, gmailAgent, taskAgent) NÃO CONSTA na lista [AGENTES ESPECIALISTAS EXECUTADOS NESTE TURNO].\n" +
  "     B) O usuário pediu múltiplas ações distintas (ex: consultar o tempo E criar uma tarefa) e uma das ações foi completamente ignorada sem que o especialista tenha sido executado.\n" +
  "     C) A resposta contradiz diretamente os dados recuperados em [DADOS COLETADOS NOS BASTIDORES].\n\n" +
  ...
  "- Se houver falha real comprovada: verdict = 'NEEDS_CORRECTION' e forneça 'feedback' cirúrgico para a Supervisora.";
...
export async function evaluatorNode(
  ...
  try {
    ...
    logger.warn(`[EVALUATOR] Resposta REPROVADA. Feedback para a supervisora: "${parsed.feedback}"`);
    return {
